...
import { downloadNutritionPlanShareImage, generateNutritionPlanShareImage } from "./nutrition-plan-share-image";
import type { NutritionPlanShareImagePayload } from "./nutrition-plan-share-image.types";
...
const nutritionCopy = {
  en: {
    ...
    downloadPlanImage: "Download image",
    edit: "Edit",
    ...
    generatePlanImage: "Generate plan image",
    generatingPlanImage: "Generating image...",
    ...
    planImageDownloaded: "Plan image downloaded.",
    planImageGeneratedBy: "Generated by athmira",
    planImageShared: "Plan image ready to share.",
    planImageUnsupported: "Image export is available on web. Open athmira.com/nutrition in a browser to download a 1:1 share image.",
    planImagePreview: "Plan image preview",
  ...
  es: {
    ...
    downloadPlanImage: "Descargar imagen",
    edit: "Editar",
    ...
    generatePlanImage: "Generar imagen del plan",
    generatingPlanImage: "Generando imagen...",
    ...
    planImageDownloaded: "Imagen del plan descargada.",
    planImageGeneratedBy: "Generado por athmira",
    planImageShared: "Imagen del plan lista para compartir.",
    planImageUnsupported: "La exportación de imagen está disponible en web. Abre athmira.com/nutrition en un navegador para descargar una imagen 1:1 compartible.",
    planImagePreview: "Previsualización de imagen",
...
function NutritionPlanViewer({
  ...
  const [shareImagePreview, setShareImagePreview] = useState<{ dataUrl: string; fileName: string } | null>(null);
  const calculations = useMemo(() => calculateDraft(draft), [draft]);
  const visibleItems = useMemo(() => draft.items.filter((item) => !isWaterProduct(item.product)), [draft.items]);
  const shareImagePayload = useMemo(
    () =>
      createNutritionPlanShareImagePayload({
        bottleCalculations: calculations.bottleCalculations,
        draft,
        items: visibleItems,
        language,
    ...
    [calculations.bottleCalculations, calculations.totals, draft, language, visibleItems]
  );
  ...
  async function generateShareImage() {
    ...
    try {
      const result = await generateNutritionPlanShareImage(shareImagePayload);

      setShareImageMessage(result.message);

      if (result.status === "generated" && result.dataUrl && result.fileName) {
        setShareImagePreview({ dataUrl: result.dataUrl, fileName: result.fileName });
      } else {
        Alert.alert(copy.generatePlanImage, result.message);
      }
    } catch (imageError) {
      const message = getErrorMessage(imageError);
      setShareImageMessage(message);
      Alert.alert(copy.generatePlanImage, message);
    } finally {
  ...
  function downloadShareImage() {
    ...
    downloadNutritionPlanShareImage(shareImagePreview.dataUrl, shareImagePreview.fileName);
    setShareImageMessage(copy.planImageDownloaded);
  }
  ...
  return (
    <View style={styles.editorStack}>
      <View style={[styles.editorHeader, compact && styles.editorHeaderMobile]}>
        ...
        <Inline>
          <Button loading={generatingImage} onPress={() => void generateShareImage()} variant="secondary">
            {generatingImage ? copy.generatingPlanImage : copy.generatePlanImage}
          </Button>
      ...
      {shareImageMessage ? <Text style={styles.shareImageStatus}>{shareImageMessage}</Text> : null}
      <NutritionPlanShareImageModal
        image={shareImagePreview}
        onClose={() => setShareImagePreview(null)}
...
function NutritionPlanShareImageModal({
  image,
  onClose,
  onDownload
}: {
  ...
  return (
    <Modal animationType="fade" onRequestClose={onClose} transparent visible={Boolean(image)}>
      <View style={styles.modalOverlay}>
        <View style={styles.shareImageModal}>
          <View style={styles.modalHeader}>
            <View>
              <Text style={styles.sectionKicker}>{copy.generatePlanImage}</Text>
              <Text style={styles.modalTitle}>{copy.planImagePreview}</Text>
            </View>
            <Button onPress={onClose} variant="secondary">
              {copy.close}
            </Button>
          ...
          {image ? (
            <View style={styles.shareImagePreviewFrame}>
              <Image
                accessibilityLabel={copy.planImagePreview}
                resizeMode="contain"
                source={{ uri: image.dataUrl }}
                style={styles.shareImagePreviewImage}
              />
            </View>
          ) : null}

          <Inline style={styles.shareImageModalActions}>
            <Button onPress={onDownload}>{copy.downloadPlanImage}</Button>
            <Button onPress={onClose} variant="secondary">
              {copy.close}
            </Button>
          </Inline>
...
function createNutritionPlanShareImagePayload({
  bottleCalculations,
  draft,
  items,
  language,
  ...
  const copy = nutritionCopy[language];
  const unitServing = language === "es" ? "porción" : "serving";
  const carbsUnit = language === "es" ? "g carbos" : "g carbs";
  const sodiumUnit = language === "es" ? "mg sodio" : "mg sodium";
  const waterText = language === "es" ? "agua restante" : "water remaining";
  const concentrationText = language === "es" ? "concentración" : "concentration";

  return {
    brand: "athmira",
    bottles: draft.bottles.map((bottle) => {
      const calculation = bottleCalculations.find((entry) => entry.bottle.id === bottle.id) ?? calculateBottleTotals(bottle, []);
      const bottleItems = items.filter((item) => item.location === "bottle" && item.bottle_id === bottle.id);

      return {
        ingredients: bottleItems.map((item, index) => {
          const calculated = calculateNutritionItem(item.product, stripDraftItem(item));

          return {
            color: getProductColor(item.product, index) ?? colors.primary,
            meta: `${formatQuantity(item.quantity)} ${item.unit ?? unitServing} / ${round(calculated.calculated_carbs, 0)} ${carbsUnit} / ${round(calculated.calculated_sodium_mg, 0)} ${sodiumUnit}`,
            name: getProductDisplayName(item.product, language)
          };
        }),
        meta: `${round(calculation.remainingWaterMl, 0)} ml ${waterText} / ${round(calculation.carbsPerLiter, 0)} g/L ${concentrationText}`,
        name: bottle.name
      };
    }),
    carriedItems: items
      .filter((item) => item.location !== "bottle")
      .map((item, index) => {
        const calculated = calculateNutritionItem(item.product, stripDraftItem(item));

        return {
          color: getProductColor(item.product, index) ?? colors.primary,
          meta: `${formatTiming(item, language)} / ${formatQuantity(item.quantity)} ${item.unit ?? unitServing} / ${round(calculated.calculated_carbs, 0)} ${carbsUnit}`,
          name: getProductDisplayName(item.product, language)
        };
      }),
    copy: {
      bottles: copy.bottlesTitle,
      carried: copy.foodCarried,
      disclaimer: copy.estimatesDisclaimer,
      domain: "athmira.com",
      downloaded: copy.planImageDownloaded,
      empty: copy.noItemsAssigned,
      generated: copy.planImageGeneratedBy,
      goals: copy.targets.toUpperCase(),
      more: (count) => (language === "es" ? `+ ${count} más` : `+ ${count} more`),
      shared: copy.planImageShared,
      strategy: copy.strategyBoard.toUpperCase(),
      unsupported: copy.planImageUnsupported
    },
    fileName: `athmira-${draft.title || copy.nutritionPlan}`,
    meta: `${getActivityLabel(draft.activity_type, language)} / ${formatDuration(draft.duration_minutes)} / ${getIntensityLabel(draft.intensity, language)}`,
    metrics: [
      {
        detail: `${round(totals.carbsPerHour, 0)} / ${draft.target_carbs_per_hour ? round(draft.target_carbs_per_hour, 0) : "--"} g/hr`,
        label: copy.totalCarbs,
        value: `${round(totals.totalCarbs, 0)} g`
      },
      {
        detail: `${round(totals.fluidsMlPerHour, 0)} / ${draft.target_fluids_ml_per_hour ? round(draft.target_fluids_ml_per_hour, 0) : "--"} ml/hr`,
        label: copy.totalFluids,
        value: `${round(totals.totalFluidsMl, 0)} ml`
      },
      {
        detail: `${round(totals.sodiumMgPerHour, 0)} / ${draft.target_sodium_mg_per_hour ? round(draft.target_sodium_mg_per_hour, 0) : "--"} mg/hr`,
        label: copy.totalSodium,
        value: `${round(totals.totalSodiumMg, 0)} mg`
      },
      {
        detail: `${round(totals.totalCalories, 0)} / ${draft.estimated_calories_burned ? round(draft.estimated_calories_burned, 0) : "--"} kcal`,
        label: copy.totalCalories,
        value: `${round(totals.totalCalories, 0)} kcal`
      }
    ],
    title: draft.title || copy.nutritionPlan
  };
}
