import type { CSSProperties } from "react";

type RiderSilhouetteOverlayProps = {
  detected: boolean;
  detectedLabel: string;
  guide: string;
  title: string;
};

const RIDER_KEYFRAMES = `
@keyframes athmira-rider-pulse {
  0%   { opacity: 0; transform: scaleX(1)  scale(0.96); }
  10%  { opacity: 1; transform: scaleX(1)  scale(1); }
  40%  { opacity: 1; transform: scaleX(1)  scale(1); }
  49%  { opacity: 0; transform: scaleX(1)  scale(0.96); }
  51%  { opacity: 0; transform: scaleX(-1) scale(0.96); }
  60%  { opacity: 1; transform: scaleX(-1) scale(1); }
  90%  { opacity: 1; transform: scaleX(-1) scale(1); }
  99%  { opacity: 0; transform: scaleX(-1) scale(0.96); }
  100% { opacity: 0; transform: scaleX(1)  scale(0.96); }
}
`;

export function RiderSilhouetteOverlay({ detected, detectedLabel, guide, title }: RiderSilhouetteOverlayProps) {
  return (
    <div style={containerStyle} aria-hidden>
      <style>{RIDER_KEYFRAMES}</style>
      <div style={{ ...overlayStyle, opacity: detected ? 0 : 1 }}>
        <div
          style={{
            ...riderWrapperStyle,
            animationPlayState: detected ? "paused" : "running"
          }}
        >
          <svg
            viewBox="0 0 24 24"
            fill="none"
            preserveAspectRatio="xMidYMid meet"
            style={svgStyle}
            role="img"
            aria-label={guide}
          >
            <path
              d="M11 18C11 18.5523 11.4477 19 12 19C12.5523 19 13 18.5523 13 18H11ZM12 13H13C13 12.7348 12.8946 12.4804 12.7071 12.2929L12 13ZM8.5 9.5L7.8415 8.74742C7.6332 8.92968 7.50976 9.19011 7.50055 9.46672C7.49134 9.74334 7.59719 10.0114 7.79289 10.2071L8.5 9.5ZM12.5 6L13.2593 5.34921C13.0855 5.1465 12.8379 5.02169 12.5716 5.00257C12.3053 4.98345 12.0424 5.07161 11.8415 5.24742L12.5 6ZM18.5 11C19.0523 11 19.5 10.5523 19.5 10C19.5 9.44772 19.0523 9 18.5 9V11ZM8 17C8 18.6569 6.65685 20 5 20V22C7.76142 22 10 19.7614 10 17H8ZM5 20C3.34315 20 2 18.6569 2 17H0C0 19.7614 2.23858 22 5 22V20ZM2 17C2 15.3431 3.34315 14 5 14V12C2.23858 12 0 14.2386 0 17H2ZM5 14C6.65685 14 8 15.3431 8 17H10C10 14.2386 7.76142 12 5 12V14ZM13 18V13H11V18H13ZM12.7071 12.2929L9.20711 8.79289L7.79289 10.2071L11.2929 13.7071L12.7071 12.2929ZM9.1585 10.2526L13.1585 6.75258L11.8415 5.24742L7.8415 8.74742L9.1585 10.2526ZM11.7407 6.65079C12.2544 7.25009 13.2032 8.3069 14.3529 9.22044C15.4669 10.1056 16.9452 11 18.5 11V9C17.6548 9 16.6331 8.47777 15.5971 7.65456C14.5968 6.85976 13.7456 5.91657 13.2593 5.34921L11.7407 6.65079ZM22 17C22 18.6569 20.6569 20 19 20V22C21.7614 22 24 19.7614 24 17H22ZM19 20C17.3431 20 16 18.6569 16 17H14C14 19.7614 16.2386 22 19 22V20ZM16 17C16 15.3431 17.3431 14 19 14V12C16.2386 12 14 14.2386 14 17H16ZM19 14C20.6569 14 22 15.3431 22 17H24C24 14.2386 21.7614 12 19 12V14ZM16 3V5C17.1046 5 18 4.10457 18 3H16ZM16 3H14C14 4.10457 14.8954 5 16 5V3ZM16 3V1C14.8954 1 14 1.89543 14 3H16ZM16 3H18C18 1.89543 17.1046 1 16 1V3Z"
              fill="#ffffff"
              fillOpacity="0.94"
            />
          </svg>
        </div>

        <div style={captionStyle}>
          <div style={captionTitleStyle}>{title}</div>
          <div style={captionTextStyle}>{guide}</div>
        </div>
      </div>

      <div style={{ ...detectedBadgeStyle, opacity: detected ? 1 : 0 }}>{detectedLabel}</div>
    </div>
  );
}

const containerStyle: CSSProperties = {
  inset: 0,
  pointerEvents: "none",
  position: "absolute"
};

const overlayStyle: CSSProperties = {
  alignItems: "center",
  display: "flex",
  flexDirection: "column",
  inset: 0,
  justifyContent: "center",
  padding: "16px",
  position: "absolute",
  transition: "opacity 400ms ease"
};

const riderWrapperStyle: CSSProperties = {
  alignItems: "center",
  animation: "athmira-rider-pulse 7s ease-in-out infinite",
  display: "flex",
  height: "72%",
  justifyContent: "center",
  maxWidth: "78%",
  transformOrigin: "center",
  willChange: "opacity, transform"
};

const svgStyle: CSSProperties = {
  filter: "drop-shadow(0 8px 22px rgba(0,0,0,0.55))",
  height: "100%",
  width: "100%"
};

const captionStyle: CSSProperties = {
  backgroundColor: "rgba(13,27,34,0.82)",
  borderRadius: 12,
  bottom: 20,
  boxShadow: "0 6px 20px rgba(0,0,0,0.35)",
  color: "#ffffff",
  maxWidth: 460,
  padding: "12px 18px",
  position: "absolute",
  textAlign: "center"
};

const captionTitleStyle: CSSProperties = {
  fontSize: 13,
  fontWeight: 900,
  letterSpacing: "0.08em",
  textTransform: "uppercase"
};

const captionTextStyle: CSSProperties = {
  fontSize: 13,
  fontWeight: 500,
  lineHeight: 1.4,
  marginTop: 5,
  opacity: 0.92
};

const detectedBadgeStyle: CSSProperties = {
  backgroundColor: "rgba(12,175,215,0.94)",
  borderRadius: 999,
  color: "#ffffff",
  fontSize: 11,
  fontWeight: 800,
  letterSpacing: "0.08em",
  padding: "6px 14px",
  position: "absolute",
  right: 14,
  textTransform: "uppercase",
  top: 14,
  transition: "opacity 300ms ease"
};
