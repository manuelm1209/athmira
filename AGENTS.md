...
- Native pose detection runs MediaPipe Pose Landmarker (Lite) via the local Expo Module at `apps/app/modules/expo-pose-landmarker/`. Swift uses the Metal GPU delegate (`MediaPipeTasksVision` pod); Kotlin uses the Android GPU delegate (`com.google.mediapipe:tasks-vision`). The shared `useMoveNetDetector` hook in `apps/app/src/features/analysis/native/` keeps its existing signature but its internals now call `detectPoseFromUri()` from the local module — no TFJS, no `jpeg-js`, no manual decode loop. Target FPS: ≥8 in M1 (polling-based), ≥20 in M2 (live-stream `PoseLandmarkerView`).
